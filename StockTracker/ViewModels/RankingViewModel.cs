...
namespace StockTracker.ViewModels
{
    ...
    public class RankingViewModel : ViewModelBase
    {
        ...
        public string BuildRankingWebsiteHtml()
        {
            ...
            html.AppendLine(".breadth-card{width:min(220px,100%);min-width:0;padding:14px;margin:0 0 16px auto;text-align:center;overflow:visible;isolation:isolate;}.breadth-title{margin:0 0 2px;color:#fff;font-size:15px;}.breadth-subtitle{margin:0;color:var(--text-muted);font-size:10px;}.breadth-donut{width:132px;height:132px;max-width:100%;aspect-ratio:1;border-radius:50%;margin:12px auto;display:grid;place-items:center;position:relative;overflow:visible;}.breadth-donut::after{content:'';position:absolute;inset:17px;background:var(--panel-bg);border-radius:50%;}.breadth-center{position:relative;z-index:1;display:flex;flex-direction:column;line-height:1.2;white-space:nowrap;}.breadth-tone{font-size:16px;font-weight:700;}.breadth-ratio{font-size:21px;font-weight:700;color:#fff;}.breadth-counts{display:flex;justify-content:center;gap:9px;font-size:12px;font-weight:600;white-space:nowrap;}.breadth-average{margin:8px 0 0;font-size:12px;color:var(--text-muted);}.breadth-note{margin:8px 0 0;color:var(--text-muted);font-size:10px;line-height:1.4;}");
            ...
            html.AppendLine(".portfolio-panel{position:fixed;z-index:50;top:76px;right:0;bottom:0;width:min(760px,calc(100vw - 24px));margin:0;border-radius:12px 0 0 0;overflow-y:auto;transform:translateX(100%);transition:transform .22s ease;box-shadow:-12px 0 28px rgba(0,0,0,.35);}.portfolio-panel.is-open{transform:translateX(0);}.portfolio-toggle{position:fixed;z-index:51;top:94px;right:0;width:46px;height:48px;border:1px solid var(--border);border-right:0;border-radius:9px 0 0 9px;background:#1f6feb;color:#fff;font-size:24px;cursor:pointer;box-shadow:-4px 2px 10px rgba(0,0,0,.25);transition:right .22s ease;}.portfolio-panel.is-open .portfolio-toggle{right:min(760px,calc(100vw - 24px));}.portfolio-toggle:hover{background:#388bfd;}.portfolio-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px;}.portfolio-head h3{margin:0;color:#f0f6fc;}.portfolio-muted{color:var(--text-muted);font-size:12px;line-height:1.6;}.portfolio-controls{display:grid;grid-template-columns:repeat(3,minmax(100px,1fr));gap:10px;align-items:end;}.portfolio-controls label{display:block;color:var(--text-muted);font-size:11px;margin-bottom:5px;}.portfolio-controls input{width:100%;box-sizing:border-box;}.portfolio-actions{display:flex;gap:8px;flex-wrap:wrap;}.portfolio-button{border:1px solid #30363d;background:#21262d;color:#c9d1d9;border-radius:7px;padding:8px 10px;cursor:pointer;}.portfolio-button.primary{background:#1f6feb;border-color:#388bfd;color:#fff;}.portfolio-summary{display:grid;grid-template-columns:repeat(2,minmax(120px,1fr));gap:10px;margin:16px 0;}.portfolio-stat{background:#0d1117;border:1px solid #30363d;border-radius:8px;padding:10px 12px;}.portfolio-stat span{display:block;color:#8b949e;font-size:11px;margin-bottom:5px;}.portfolio-stat strong{font-size:17px;color:#f0f6fc;}.portfolio-table{width:100%;border-collapse:collapse;font-size:13px;}.portfolio-table th,.portfolio-table td{padding:10px 8px;border-bottom:1px solid #21262d;text-align:right;white-space:nowrap;}.portfolio-table th{color:#8b949e;font-size:11px;}.portfolio-table th:first-child,.portfolio-table td:first-child{text-align:left;}.portfolio-row{cursor:pointer;}.portfolio-row:hover{background:#21262d;}.portfolio-advice{font-weight:600;}.portfolio-empty{padding:22px;text-align:center;color:#8b949e;}@media(max-width:768px){.portfolio-panel{top:60px;width:calc(100vw - 12px);}.portfolio-panel.is-open .portfolio-toggle{right:calc(100vw - 12px);}.portfolio-controls{grid-template-columns:1fr 1fr;}.portfolio-summary{grid-template-columns:1fr 1fr;}.portfolio-head{flex-direction:column;}.portfolio-table-wrap{overflow-x:auto;}}");
            html.AppendLine("</style>");
            html.AppendLine("</head>");
            ...
            html.AppendLine("  <div class='portfolio-summary' id='portfolioSummary'></div><div class='portfolio-table-wrap'><table class='portfolio-table'><thead><tr><th>&#25345;&#32929;</th><th>&#29694;&#20729;</th><th>&#20170;&#26085;&#28450;&#36300;</th><th>&#25613;&#30410;&#29575;</th><th>&#29694;&#26377;&#27402;&#37325;</th><th>&#30446;&#27161;&#27402;&#37325;</th><th>&#24314;&#35696;</th><th></th></tr></thead><tbody id='portfolioBody'></tbody></table></div>");
            html.AppendLine("</div>");
            ...
            html.AppendLine("function calculatePortfolioTargets(items,reserve,limit){const budget=Math.max(0,100-reserve),cap=Math.min(100,Math.max(0,limit));const active=items.filter(x=>x.stock&&x.stock.score>=65&&x.stock.crash<=45&&x.stock.price>0).map(x=>({item:x,signal:x.stock.score*Math.max(.1,1-x.stock.crash/100)}));let remaining=budget,available=[...active],targets=new Map();while(available.length&&remaining>.0001){const sum=available.reduce((n,x)=>n+x.signal,0);const capped=available.filter(x=>remaining*x.signal/sum>cap+.0001);if(!capped.length){available.forEach(x=>targets.set(x.item.symbol,remaining*x.signal/sum));break;}capped.forEach(x=>{targets.set(x.item.symbol,cap);remaining-=cap;});available=available.filter(x=>!capped.includes(x));}return targets;}");
