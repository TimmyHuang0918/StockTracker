...
namespace StockTracker.ViewModels
{
    ...
    public class RankingViewModel : ViewModelBase
    {
        ...
        public string BuildRankingWebsiteHtml()
        {
            ...
            html.AppendLine("function refreshMarketGroupCards(){const host=document.getElementById('marketGroupCards');if(!host)return;const buckets=new Map();rawData.forEach(stock=>{const mapping=groupMappingBySymbol.get(String(stock.symbol));if(!mapping)return;const names=[...new Set([mapping.Industry??mapping.industry,...(mapping.Themes??mapping.themes??[]),...(mapping.CoreThemes??mapping.coreThemes??[])].map(normalizeMarketGroupName).filter(Boolean))];names.forEach(name=>{const items=buckets.get(name)||[];items.push(stock);buckets.set(name,items);});});host.replaceChildren();[...buckets.entries()].sort((a,b)=>a[0].localeCompare(b[0],'zh-Hant')).forEach(([name,items])=>{const total=items.length,up=items.filter(x=>Number(x.chg)>0).length,down=items.filter(x=>Number(x.chg)<0).length,average=items.reduce((sum,x)=>sum+Number(x.chg||0),0)/total,ratio=total?up/total*100:0,style=average>0?'rise':average<0?'fall':'flat',label=average>0?'偏多':average<0?'偏空':'持平',button=document.createElement('button');button.type='button';button.className='group-filter';button.dataset.group=name;button.style.cssText='text-align:left;color:inherit;border:1px solid var(--border);border-radius:8px;padding:10px;background:rgba(139,148,158,.06);cursor:pointer';button.innerHTML=\"<div style='display:flex;justify-content:space-between;gap:8px'><strong></strong><span class='\"+style+\"'>\"+label+\"</span></div><div style='font-size:20px;font-weight:700;margin:6px 0' class='\"+style+\"'>\"+(average>0?'+':'')+average.toFixed(2)+\"%</div><div class='muted' style='font-size:12px'>上漲 \"+up+\" / 下跌 \"+down+\" / 共 \"+total+\"</div><div class='muted' style='font-size:12px;margin-top:3px'>上漲率 \"+ratio.toFixed(0)+\"%</div>\";button.querySelector('strong').textContent=name;host.append(button);});}");
