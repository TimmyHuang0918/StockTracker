...
namespace StockTracker.ViewModels
{
    ...
    public class RankingViewModel : ViewModelBase
    {
        ...
        public string BuildRankingWebsiteHtml()
        {
            ...
            html.AppendLine("  <div class='portfolio-actions' style='margin:0 0 10px'><button class='portfolio-button' id='btnPortfolioResetAll' type='button'>&#37325;&#35373;&#25152;&#26377;&#32000;&#37636;</button></div>");
            ...
            html.AppendLine("$('btnPortfolioResetAll').addEventListener('click',()=>{if(!confirm('這會清除所有持股、入出金、買賣交易、融資批次與損益調整紀錄，且無法復原。確定要重設嗎？'))return;portfolio.holdings=[];portfolio.cashFlows=[];portfolio.trades=[];portfolio.realizedAdjustments=[];portfolio.marginLots=[];portfolio.cash=0;savePortfolio();renderPortfolio();});");
            html.AppendLine("document.addEventListener('click',event=>{if(event.target?.id!=='btnPortfolioSave')return;event.stopImmediatePropagation();const symbol=$('portfolioSymbol').value.trim().toUpperCase(),shares=portfolioNumber($('portfolioShares').value),cost=portfolioNumber($('portfolioCost').value);if(!symbol||shares<=0||cost<=0){alert('請填寫股票代號、持股數與平均成本。');return;}const index=portfolio.holdings.findIndex(h=>String(h.symbol).toUpperCase()===symbol),previous=index>=0?portfolio.holdings[index]:null,delta=(previous?portfolioNumber(previous.shares)*portfolioNumber(previous.cost):0)-shares*cost;if(delta<0&&portfolio.cash<-delta){alert('可用現金不足以完成這次持股增加。');return;}portfolio.cash+=delta;$('portfolioCash').value=portfolio.cash;const holding={symbol,shares,cost};if(index>=0)portfolio.holdings[index]=holding;else portfolio.holdings.push(holding);$('portfolioSymbol').value='';$('portfolioShares').value='';$('portfolioCost').value='';savePortfolio();renderPortfolio();},true);");
            html.AppendLine("function installAutoCash(){portfolio.cashFlows=Array.isArray(portfolio.cashFlows)?portfolio.cashFlows:[];portfolio.trades=Array.isArray(portfolio.trades)?portfolio.trades:[];if(!portfolio.cashFlows.length&&portfolio.cash){portfolio.cashFlows.push({date:new Date().toISOString().slice(0,10),amount:portfolioNumber(portfolio.cash)});}const tradeCashImpact=trade=>{if(Object.prototype.hasOwnProperty.call(trade,'cashImpact')&&Number.isFinite(Number(trade.cashImpact)))return Number(trade.cashImpact);return trade.type==='sell'?portfolioNumber(trade.quantity)*portfolioNumber(trade.price)-portfolioNumber(trade.fee)-portfolioNumber(trade.tax):-(portfolioNumber(trade.quantity)*portfolioNumber(trade.price)+portfolioNumber(trade.fee)+portfolioNumber(trade.tax));};const calculate=()=>portfolio.cashFlows.reduce((sum,x)=>sum+portfolioNumber(x.amount),0)+portfolio.trades.reduce((sum,x)=>sum+tradeCashImpact(x),0);const input=$('portfolioCash');input.readOnly=true;input.title='由入金／出金與交易紀錄自動結算';const baseRender=renderPortfolio;renderPortfolio=function(){portfolio.cash=Math.max(0,calculate());input.value=portfolio.cash;baseRender();};renderPortfolio();}installAutoCash();");
            ...
            html.AppendLine("function installPortfolioDecisionSummary(){const baseRender=renderPortfolio;renderPortfolio=function(){baseRender();const items=portfolio.holdings.map(h=>{const stock=findPortfolioStock(h.symbol);return {symbol:String(h.symbol),value:stock?portfolioNumber(h.shares)*stock.price:0};}).sort((a,b)=>b.value-a.value);const total=items.reduce((sum,x)=>sum+x.value,0)+portfolioNumber(portfolio.cash);const top=items[0];let note=top?`最大持股 ${top.symbol} 占 ${total?(top.value/total*100).toFixed(1):'0.0'}%。`: '尚未建立持股。';let card=document.getElementById('portfolioDecisionRisk');if(!card){card=document.createElement('div');card.id='portfolioDecisionRisk';card.className='portfolio-muted';card.style.cssText='margin:10px 0;padding:10px;border:1px solid var(--border);border-radius:8px;background:rgba(245,193,93,.08)';document.querySelector('.portfolio-summary').after(card);}card.innerHTML=`<b style='color:#f4c15d'>部位風險</b>　${note}　請優先依「建議」欄檢視加碼、減碼或續抱。`;};renderPortfolio();}installPortfolioDecisionSummary();");
            ...
            html.AppendLine("</script>");
            html.AppendLine("<script src='portfolio-enhancements.js?v=3'></script>");
